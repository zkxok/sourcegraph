...
// FileMatch is a resolver for the GraphQL type `FileMatch`
...
func textSearch(ctx context.Context, repo gitserver.Repo, commit api.CommitID, p *PatternInfo, fetchTimeout time.Duration) (matches []*FileMatch, limitHit bool, err error) {
	...
	for {
		...
		searcherURL, err := Providers().SearcherURLs.Get(consistentHashKey, excludedSearchURLs)
		if err != nil {
			return nil, false, err
		}
		...
		if searcherURL == "" {
			...
			searcherURL, err = Providers().SearcherURLs.Get(consistentHashKey, nil)
			if err != nil {
				return nil, false, err
			}
		}
...
func textSearchURL(ctx context.Context, url string) ([]*FileMatch, bool, error) {
...
var mockSearchFilesInRepo func(ctx context.Context, repo *types.Repo, gitserverRepo gitserver.Repo, rev string, info *PatternInfo, fetchTimeout time.Duration) (matches []*FileMatch, limitHit bool, err error)

func searchFilesInRepo(ctx context.Context, repo *types.Repo, gitserverRepo gitserver.Repo, rev string, info *PatternInfo, fetchTimeout time.Duration) (matches []*FileMatch, limitHit bool, err error) {
...
func zoektSearchHEAD(ctx context.Context, query *PatternInfo, repos []*RepositoryRevisions, useFullDeadline bool) (fm []*FileMatch, limitHit bool, reposLimitHit map[string]struct{}, err error) {
	if len(repos) == 0 {
