...
type Searcher interface {
	Results(context.Context) (*SearchResults, error)
	Suggestions(context.Context, *SearchSuggestionsArgs) ([]*SearchSuggestion, error)
	//lint:ignore U1000 is used by graphql via reflection
	Stats(context.Context) (*SearchResultsStats, error)
}

// Search provides search results and suggestions.
func Search(query string) (Searcher, error) {
	q, err := query.ParseAndCheck(query)
	if err != nil {
		log15.Debug("search failed to parse", "query", query, "error", err)
		return nil, err
	}
...
// searcher is a resolver for the GraphQL type `Search`
...
func (r *searcher) rawQuery() string {
	return r.query.Syntax.Input
}

func (r *searcher) countIsSet() bool {
...
func (r *searcher) maxResults() int32 {
	count, _ := r.query.StringValues(query.FieldCount)
	if len(count) > 0 {
...
func (r *searcher) resolveRepositories(ctx context.Context, effectiveRepoFieldValues []string) (repoRevs, missingRepoRevs []*RepositoryRevisions, repoResults []*searchSuggestion, overLimit bool, err error) {
...
func getRevsForMatchedRepo(repo api.RepoName, pats []patternRevspec) (matched []RevisionSpecifier, clashing []RevisionSpecifier) {
...
func resolveRepositories(ctx context.Context, op resolveRepoOp) (repoRevisions, missingRepoRevisions []*RepositoryRevisions, repoResolvers []*SearchSuggestion, overLimit bool, err error) {
	...
	repoResults = make([]*SearchSuggestion, 0, len(repos))
	tr.LazyPrintf("Associate/validate revs - start")
	for _, repo := range repos {
		repoRev := &RepositoryRevisions{Repo: repo}
		...
		repoResults = append(repoResults, newSearchResult(
			repo,
			math.MaxInt32,
		))
	...
	return repoRevisions, missingRepoRevisions, repoResults, overLimit, nil
}
...
func (r *searcher) suggestFilePaths(ctx context.Context, limit int) ([]*SearchSuggestion, error) {
	...
	for i, file := range fileResults {
		...
		suggestions = append(suggestions, newSearchResult(file, assumedScore))
	}
...
func newSearchResult(result interface{}, score int) *SearchSuggestion {
	switch r := result.(type) {
	case *types.Repo:
		return &SearchSuggestion{Repo: r, Score: score, Length: len(r.repo.Name), Label: string(r.repo.Name)}

	case *FileMatch:
		return &SearchSuggestion{FileMatch: r, Score: score, Length: len(r.path), Label: r.path}

	case *symbols.Symbol:
		return &SearchSuggestion{Symbol: r, score: score, length: len(r.symbol.Name + " " + r.symbol.Parent), label: r.symbol.Name + " " + r.symbol.Parent}
...
// Providers returns instances of our search providers.
func Providers() *SearchProviders {
	searchOnce.Do(func() {
		// Zoekt
		index := &searchbackend.Zoekt{}
		if zoektAddr != "" {
