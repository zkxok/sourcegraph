...
func commitURL(repo *types.Repo, commit *git.Commit) string {
	return repoURL(repo) + "/-/commit/" + string(commit.ID)
}

func repoURL(repo *types.Repo) string {
	return "/" + string(repo.Name)
}

var mockSearchCommitDiffsInRepo func(ctx context.Context, repoRevs RepositoryRevisions, info *PatternInfo, query *query.Query) (results []*CommitSearchResult, limitHit, timedOut bool, err error)

func searchCommitDiffsInRepo(ctx context.Context, repoRevs RepositoryRevisions, info *PatternInfo, query *query.Query) (results []*CommitSearchResult, limitHit, timedOut bool, err error) {
...
var mockSearchCommitLogInRepo func(ctx context.Context, repoRevs RepositoryRevisions, info *PatternInfo, query *query.Query) (results []*CommitSearchResult, limitHit, timedOut bool, err error)

func searchCommitLogInRepo(ctx context.Context, repoRevs RepositoryRevisions, info *PatternInfo, query *query.Query) (results []*CommitSearchResult, limitHit, timedOut bool, err error) {
...
func searchCommitsInRepo(ctx context.Context, op commitSearchOp) (results []*CommitSearchResult, limitHit, timedOut bool, err error) {
	...
	for i, rawResult := range rawResults {
		...
		results[i] = &CommitSearchResult{Repo: repo, Commit: commit}

		&results[i].refs = rawResult.Refs
		&results[i].sourceRefs = rawResult.SourceRefs
		...
		results[i].label, err = createLabel(rawResult, commit)
		if err != nil {
			return nil, false, false, err
		}
		...
		match := &SearchResultMatch{Body: matchBody, Highlights: matchHighlights, URL: url}
		matches := []*SearchResultMatch{match}
...
func createLabel(rawResult *git.LogCommitSearchResult, repo *types.Repo, commit *git.Commit) (string, error) {
	message := commitSubject(commit.Message)
	author := commit.Author.Name
	repoName := displayRepoName(repo.Name)
...
var mockSearchCommitDiffsInRepos func(args *Args) ([]*SearchResult, *searchResultsCommon, error)

// searchCommitDiffsInRepos searches a set of repos for matching commit diffs.
func searchCommitDiffsInRepos(ctx context.Context, args *Args) ([]*SearchResult, *searchResultsCommon, error) {
...
var mockSearchCommitLogInRepos func(args *Args) ([]*SearchResult, *searchResultsCommon, error)

// searchCommitLogInRepos searches a set of repos for matching commits.
func searchCommitLogInRepos(ctx context.Context, args *Args) ([]*SearchResult, *searchResultsCommon, error) {
