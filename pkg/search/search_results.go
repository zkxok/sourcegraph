...
func (sr *SearchResults) DynamicFilters() []*SearchFilter {
...
func (r *searchResolver) doResults(ctx context.Context, forceOnlyResultType string) (res *SearchResults, err error) {
	...
	if len(repos) == 0 {
		...
		return &SearchResults{alert: alert, start: start}, nil
	}
	if overLimit {
			...
			goroutine.Go(func() {
				...
				for _, symbolFileMatch := range symbolFileMatches {
					...
					if m, ok := fileMatches[key]; ok {
						...
						resultsMu.Lock()
						results = append(results, &SearchResult{fileMatch: symbolFileMatch})
						resultsMu.Unlock()
					}
			...
			goroutine.Go(func() {
				...
				for _, r := range fileResults {
					...
					if ok {
						...
						fileMatches[key] = r
						resultsMu.Lock()
						results = append(results, &SearchResult{fileMatch: r})
						resultsMu.Unlock()
					}
					fileMatchesMu.Unlock()
				}
...
// SearchResult is a resolver for the GraphQL union type `SearchResult`
//
...
func compareSearchResults(a, b *SearchResult) bool {
