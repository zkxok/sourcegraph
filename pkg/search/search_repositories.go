...
var mockSearchRepositories func(args *Args) ([]*SearchResult, *SearchResultsCommon, error)
...
func searchRepositories(ctx context.Context, args *Args, limit int32) (res []*SearchResult, common *SearchResultsCommon, err error) {
	...
	for _, repo := range args.Repos {
		...
		if pattern.MatchString(string(repo.Repo.Name)) {
			results = append(results, &SearchResult{repo: repo.Repo, icon: repoIcon})
		}
	}
