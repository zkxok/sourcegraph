...
const STATUS_ICONS: Record<PullRequest['status'], React.ComponentType<{ className?: string }>> = {
    open: SourcePullIcon,
                        ...
                        {pull.status !== 'pending' && (
                            <li>
                                <small className="text-muted">
                                    #{pull.number} updated {formatDistance(Date.parse(pull.updatedAt), Date.now())} ago
                                    by <strong>{pull.updatedBy}</strong>
                                </small>
                            </li>
                        )}
